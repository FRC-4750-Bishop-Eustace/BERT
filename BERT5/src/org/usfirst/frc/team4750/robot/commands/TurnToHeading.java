 ...
 * 
 * The basic idea here is to take in an amount to turn in degrees. Then, use
 * what's called a PID controller to control how fast we need the motors to turn
 * to change the heading. The closer we are to the target heading the slower we
 * command the motors so we don't overshoot.
 * 
 */
public class TurnToHeading extends Command {
	 ...
	 * 
	 * @param offset
	 *            degrees we need to turn to reach goal. Often this will come
	 *            from the vision system. Negative numbers indicate a left turn,
	 *            positive are a right turn.
	 */
	...
	protected void oldinitialize() {
		...
		// READ FROM IMU!!! Note, the heading returned here is absolute!!! Not
		// relative.
	...
	protected void execute() {
		...
		// the farther off the target we are, the higher we need to set the
		// motors
		// speed/1.0 = degreesofftarget/180;
		float speed = (offset / 180.0f) * 1.0f;

		// note that there IS a minimum power setting or else we won't turn at
		// all (motors will stall)
		// so if we're commanding less than say .25, set it to .25 and then
		// adjust the sign to match what it was.
	...
	protected void oldexecute() {
		...
		// find the difference between where we are now and where we're trying
		// to get to

		/*
		 * Target Last Offset ------- ---- ------ 340 10 -30 10 340 30 120 190
		 * -70 190 120 70
		 */

		/*
		 * If absolute value of Target - currentheading < 180 then offset =
		 * target-currentheading. else if target-heading > 0 then offset =
		 * target-heading-360 else if target-heading < 0 then offset = target -
		 * heading + 360. else if target-heading == 0 then offset = 0;
		 */
		...
		// note that there IS a minimum power setting or else we won't turn at
		// all (motors will stall)
		// so if we're commanding less than say .2, set it to .2 and then adjust
		// the sign to match what it was.
		if (Math.abs(speed) < .25) {
			if (speed < 0)
				speed = -0.25f;
			else
		...
		// Currentheading:"+lastheadingread+" Offset:"+offset+" Speed:"+speed);
		Robot.driveTrain.setDriveMotors(speed, speed);
	}
	...
	protected boolean isFinished() {
		...
		// see if we're within 2 degrees of the target. That should be close
		// enough.
